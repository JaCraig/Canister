...
namespace Canister.Tests.ExtensionMethods
{
    ...
    public class ServiceCollectionAddByAttribute
    {
        ...
        [Fact]
        public void ServiceCollectionAddByAttributeTestExplicitAssemblies()
        {
            var services = new ServiceCollection();
            _ = services.AddCanisterModules(x => x.AddAssembly(typeof(ServiceCollectionAddByAttribute).Assembly));
            ServiceProvider provider = services.BuildServiceProvider();
